...
/**
 * pages/admin/games/slots.tsx
 * Slots bet history + SlotsLock panel.
 */
...
import { useSlotsLock } from '../../../hooks/useAdmin';

const MODE_OPTS = [
  { value: 'no_win',  label: 'No Win — all spins return a loss' },
  { value: 'min_win', label: 'Min Win — only minimum-payout wins allowed' },
];

// ── Slots Lock Panel ──────────────────────────────────────────────────────────

function SlotsLockPanel() {
  const { state, auditLog, loading, busy, enable, disable, refetch } = useSlotsLock();
  const { show, Toast } = useToast();
  const [modal, setModal]               = useState(false);
  const [mode, setMode]                 = useState<'no_win' | 'min_win'>('no_win');
  ...
  const handleDisable = async () => {
    ...
    if (res?.success) { show('Slots lock disabled', 'ok'); setDisableModal(false); refetch(); }
    else              { show((res as any)?.error ?? 'Failed', 'err'); }
  };
  ...
  const modeDesc: Record<string, string> = {
    no_win:  'Every spin is forced to return a loss. No payouts will be issued.',
    min_win: 'Only the minimum payout tier is allowed. Big wins and jackpots are suppressed.',
  };
  ...
  return (
    <>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Enable Slots Lock">
        <div className="space-y-4">
          <Select label="Mode" value={mode} onChange={v => setMode(v as any)} options={MODE_OPTS} />
          <Input label="Games Limit (blank = unlimited)" value={gamesLimit} onChange={setGamesLimit} type="number" placeholder="e.g. 100" />
      ...
      <Modal open={disableModal} onClose={() => setDisableModal(false)} title="Disable Slots Lock">
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Normal slot outcomes will be restored on the next spin.</p>
...
function SlotsHistory() {
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/games/slots/bets?page=${page}&limit=25`, {
  ...
  return (
    <Card>
      <SectionTitle>Recent Slot Spins</SectionTitle>
      <Table
        cols={['User', 'Bet', 'Symbols', 'Multiplier', 'Payout', 'Locked?', 'Date']}
        loading={loading}
        empty="No spins yet."
      >
        {spins.map(s => (
          <TR key={s._id}>
            ...
            <TD>
              <span className="font-mono text-gray-400 text-[10px]">
                {Array.isArray(s.symbols) ? s.symbols.join(' ') : (s.result ?? '—')}
              </span>
            </TD>
...
const AdminSlots: NextPage = () => (
  <>
    <Head>
      <title>Slots — DashBets Admin</title>
